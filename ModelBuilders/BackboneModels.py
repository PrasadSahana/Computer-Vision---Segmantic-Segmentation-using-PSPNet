#This code shows how many backbone models are supported for PSPNet for indoor images
...
def build_frontend(inputs, frontend, is_training=True, pretrained_dir="models"):
    if frontend == 'ResNet50':
        ...
        with slim.arg_scope(resnet_v2.resnet_arg_scope()):
            ...
            init_fn = slim.assign_from_checkpoint_fn(model_path=os.path.join(pretrained_dir, 'resnet_v2_101.checkpoint'), var_list=slim.get_model_variables('resnet_v2_101'), ignore_missing_vars=True)
        ...
        raise ValueError("Unsupported model for PSPNet '%s'. This function only supports ResNet50, ResNet101,  and ResNet152" % (frontend))
